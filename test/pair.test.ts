import { ChainId, Token, Pair, Fetcher, TokenAmount, JSBI, Price, UniswapV2RoutablePlatform } from '../src'
...
describe('Pair', () => {
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      const usdc = new Token(ChainId.MAINNET, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 18, 'USDC', 'USD Coin')
      const dai = new Token(ChainId.MAINNET, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')
      expect(Pair.getAddress(usdc, dai, UniswapV2RoutablePlatform.SWAPR)).toEqual(
        '0x0e18852eE5CE266E2Ce6f4844Ba04cd9CD11AF5B'
      )
    })
  })
  ...
  describe('#getAddressfromWrongPlatform', () => {
    it('returns the incorrect address', () => {
      const usdc = new Token(ChainId.MAINNET, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 18, 'USDC', 'USD Coin')
      const dai = new Token(ChainId.MAINNET, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')
      expect(Pair.getAddress(usdc, dai, UniswapV2RoutablePlatform.UNISWAP)).not.toEqual(
        '0x0e18852eE5CE266E2Ce6f4844Ba04cd9CD11AF5B'
      )
    })
  })
  ...
  describe('#getAddressFromUniswap', () => {
    it('returns the correct address from Uniswap rather than Swapr', () => {
      ...
      expect(Pair.getAddress(usdc, weth, UniswapV2RoutablePlatform.UNISWAP)).toEqual(
  ...
  describe('#getAddressFromSushiswap', () => {
    it('returns the correct address from Uniswap rather than Swapr', () => {
      const usdc = new Token(ChainId.MAINNET, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 18, 'USDC', 'USD Coin')
      const weth = new Token(ChainId.MAINNET, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', 18, 'WETH', 'Wrapped ETH')
      expect(Pair.getAddress(usdc, weth, UniswapV2RoutablePlatform.SUSHISWAP)).toEqual(
  ...
  describe('#token0Price', () => {
    it('returns price of token0 in terms of token1', () => {
      expect(new Pair(new TokenAmount(USDC, '101'), new TokenAmount(DAI, '100')).token0Price).toEqual(
        new Price({ baseCurrency: DAI, quoteCurrency: USDC, denominator: '100', numerator: '101' })
      )
  ...
  describe('#token1Price', () => {
    it('returns price of token1 in terms of token0', () => {
      expect(new Pair(new TokenAmount(USDC, '101'), new TokenAmount(DAI, '100')).token1Price).toEqual(
        new Price({ baseCurrency: USDC, quoteCurrency: DAI, denominator: '101', numerator: '100' })
      )
