  ...
  static async getQuote(
    ...
    if (!amountOut.eq('0')) {
      try {
        ...
        const sendFromAmount = amount.raw
      ...
      const currencyAmountIn =
        tradeType === TradeType.EXACT_INPUT ? amount : new TokenAmount(wrappedQuote, parsedAmount)
      const currencyAmountOut =
        tradeType === TradeType.EXACT_INPUT ? new TokenAmount(wrappedQuote, parsedAmount) : amount
