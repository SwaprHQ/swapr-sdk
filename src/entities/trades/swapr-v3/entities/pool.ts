...
export class Pool {
  ...
  public async getInputAmount(
    ...
    const {
      ...
      liquidity,
      tickCurrent,
    } = await this.swap(zeroForOne, JSBI.multiply(outputAmount.quotient, JSBI.BigInt(-1)), sqrtPriceLimitX96)
  ...
  private async swap(
    ...
    const exactInput = JSBI.greaterThanOrEqual(amountSpecified, JSBI.BigInt(0))
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, JSBI.BigInt(0)) && state.sqrtPriceX96 != sqrtPriceLimitX96) {
